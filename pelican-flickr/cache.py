...
class FlickrCache:
    ...
    def __init__(self):
        ...
        self.api = flickrapi.FlickrAPI(
            pelicanflickr.FLICKR_API_KEY,
            pelicanflickr.FLICKR_API_SECRET
        )

        # Setup includes / excludes
        self.sets_exclude = pelicanflickr.FLICKR_SETS_EXCLUDE
    ...
    def load_photosets(self):
        ...
        try:
            ...
            sets = self.api.photosets_getList(
                user_id=pelicanflickr.FLICKR_USER
            )
            for photoset in sets.find('photosets').findall('photoset'):
                s = FlickrPhotoset(xml=photoset)
                if self.sets_exclude and (
                    s.id in self.sets_exclude or
                    s.title in self.sets_exclude
                ):
                    logger.info(u"Excludes %s" % (s,))
                    continue
                logger.info(u"Use %s from %s" %
                            (s, s.cached and 'cache' or 'flickr'))
            ...
            self.sets_cached.data = [ss.id for ss in self.sets]
            self.sets_cached.save()
        except Exception as e:
            logger.warning("Failed to load photosets from Flickr: %s" %
                           (str(e), ))
